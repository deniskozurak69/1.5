...
namespace LibraryWebApplication1.Controllers
{
    ...
    public class ChartController : ControllerBase
    {
        ...
        [HttpGet("CommentsByArticles")]
        public JsonResult CommentsByArticles()
        {
            var articles = _context.Articles.ToList();
            List<object> arComment = new List<object>();
            arComment.Add(new[] { "Стаття", "Кількість коментарів" });
            int cnt;
            foreach (var c in articles)
            {
                var comments = _context.Comments.Where(a => a.ArticleId == c.ArticleId);
                cnt = comments.Count();
                arComment.Add(new object[] { c.ArticleName, cnt });
            }
            return new JsonResult(arComment);
        }
    }
}
